...
export const toggleCakeLike = async (cakeItemId) => {
    ...
    const response = await jwtAxios.post(`${baseUrl}/cake/toggle`, payload);
    return response.data;
};

/* 특정 케이크 상품의 찜 여부를 확인 */
export const getCakeLikeStatus = async (cakeItemId) => {
    const response = await jwtAxios.get(`${baseUrl}/cake/status/${cakeItemId}`);
    return response.data;
};

/* 로그인한 사용자가 찜한 모든 케이크 상품 목록을 조회 */
export const getLikedCakeItems = async () => {
    const response = await jwtAxios.get(`${baseUrl}/cake`);
    return response.data;
};
...
export const toggleShopLike = async (shopId) => {
    ...
    const response = await jwtAxios.post(`${baseUrl}/shop/toggle`, payload);
    return response.data;
};

/* 특정 매장의 찜 여부를 확인 */
export const getShopLikeStatus = async (shopId) => {
    const response = await jwtAxios.get(`${baseUrl}/shop/status/${shopId}`);
    return response.data;
};

/* 로그인한 사용자가 찜한 모든 매장 목록을 조회 */
export const getLikedShops = async () => {
    const response = await jwtAxios.get(`${baseUrl}/shop`);
    return response.data;
};