...
export const createOrder = async (payload) => { // async 키워드 추가
    const response =  await jwtAxios.post(`${baseUrl}/buyer/orders/create`, payload);
...
export const getOrderList = async ({ page = 1, size = 10 } = {}) => {
    ...
    const response = await jwtAxios.get(`${baseUrl}/buyer/orders`, {
        params: { page, size }
    });
...
export const getOrderDetail = async (orderId) => {
    const response = await jwtAxios.get(`${baseUrl}/buyer/orders/${orderId}`);
    return response.data;
};
